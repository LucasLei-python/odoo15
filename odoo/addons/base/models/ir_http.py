...
class IrHttp(models.AbstractModel):
    ...
    @classmethod
    def binary_content_crm(cls, xmlid=None, model='xlcrm.document', id=None, field='db_datas', unique=False,
                           filename=None,
        ...
        if model == 'xlcrm.documents' and obj.type == 'url' and obj.url:
        ...
        if not mimetype:
            ...
            if not mimetype and getattr(env[model]._fields[field], 'xlcrm.documents', False):
                # for binary fields, fetch the ir_attachement for mimetype check
                attach_mimetype = env['xlcrm.documents'].search_read(
