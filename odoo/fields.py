...
class Datetime(Field):
    ...
    def now(*args):
        ...
        return datetime.now().replace(microsecond=0) + timedelta(hours=8)

    @staticmethod
    def utc_now(record, *args):
        sysstr = platform.system()
        if sysstr == "Windows":
            return datetime.now().replace(microsecond=0) + timedelta(hours=8)
        else:
            return Datetime.context_timestamp(record, datetime.now() + timedelta(hours=0))
